...
{
    public partial class Report_ChallanWiseValuationPrintFilter : System.Web.UI.Page
    {
        ...
        private void retriveData()
        {
            try
            {
                // dadapter = new SqlDataAdapter("select distinct IndentingDivisionName from DeliveryItemsChallan where IsDeliveredTemporary = 'No' and ChallanDate between '" + stDate + "' and  '" + edDate + "' ", con);
                 dadapter = new SqlDataAdapter("select distinct IssueHeadName from DeliveryItemsChallan,DeliveryItemsDetails "+
                     " where DeliveryItemsChallan.DeliveryItemsChallanID=DeliveryItemsDetails.DeliveryItemsChallanID and" +
                     " IsDeliveredTemporary = 'No' and ChallanDate between '" + stDate + "' and  '" + edDate + "' ", con);
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                SqlCommand cmd = new SqlCommand("Select DISTINCT  DeliveryItemsChallan.DeliveryItemsChallanID,ChargeableHeadName, " +
                    " ChallanDate,IndentReference,IndentDate,TotalAmount " +
                    " from DeliveryItemsChallan,DeliveryItemsDetails  where " +
                    " DeliveryItemsChallan.DeliveryItemsChallanID=DeliveryItemsDetails.DeliveryItemsChallanID and  "+
                    " IssueHeadName='" + DEP_ID.Text.ToString() + "' and ChallanDate between '" + stDate + "' and  '" + edDate + "'  order by ChargeableHeadName", con);
