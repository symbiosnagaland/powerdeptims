...
{
    public partial class ReceiptEntriesList : System.Web.UI.Page
    {
        ...
        private void retriveData()
        {
            try
            {
                dadapter = new SqlDataAdapter("SELECT * FROM [transformer_receipts] ORDER BY [challanno] DESC", con);
                dset = new DataSet();
                dadapter.Fill(dset);
        ...
        protected void btnSearchImage_ServerClick(object sender, EventArgs e)
        {
            try
            {
                SqlDataAdapter ss;
                DataSet xx;
                ss = new SqlDataAdapter("SELECT * FROM [transformer_receipts] where challanno='" + _txtsearch.Value + "' ", con);
