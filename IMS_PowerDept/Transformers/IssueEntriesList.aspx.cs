...
namespace IMS_PowerDept.Transformers
{
    public partial class IssueEntriesList : System.Web.UI.Page
    {
        ...
        private void retriveData()
        {
            try
            {
                dadapter = new SqlDataAdapter("SELECT * FROM [transformer_issue] ORDER BY [challanno] DESC", con);
                dset = new DataSet();
                dadapter.Fill(dset);
        ...
        protected void btnSearchImage_ServerClick(object sender, EventArgs e)
        {
            try
            {
                SqlDataAdapter ss;
                DataSet xx;
                ss = new SqlDataAdapter("SELECT * FROM [transformer_issue] where challanno='" + _txtsearch.Value + "' ", con);
