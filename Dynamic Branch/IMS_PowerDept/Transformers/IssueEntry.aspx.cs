...
namespace IMS_PowerDept.Transformers
{
    ...
    {
        protected static DataTable dtOilType = new DataTable();
        protected static DataTable dtTransformers = new DataTable();        
            ...
            {
                //retrive divisions
                string cmdText = "SELECT DISTINCT divisionName, division FROM Divisions";
                ...
                _ddIntendDivisions.DataValueField = "division";
            ...
            public void FillTransformerMasterOilType()
            {
                ...
                if (File.Exists(path))
                {
                    ...
                    try
                    {
                        ...
                        OleDbCommand dbCommandOilTypes = new OleDbCommand("SELECT * FROM [OILTYPE$]", dbConnection);
                        ...
                        OleDbDataReader dbReader = dbCommandOilTypes.ExecuteReader();
                        dtOilType.Load(dbReader);
            ...
            protected void gvItems_RowDataBound(object sender, GridViewRowEventArgs e)
            {
                ...
                if (dtOilType == null || dtOilType.Rows.Count <= 0)
                    FillTransformerMasterOilType();
                ...
                else //if (dtTransformer != null)
                {
                    ...
                    try
                    {
                        if (e.Row.RowType == DataControlRowType.DataRow)
                        {
                            ...
                            DropDownList ddlOilType = (e.Row.FindControl("ddlOilType") as DropDownList);
                            ddlOilType.DataSource = dtOilType;
                            ddlOilType.DataValueField = "OILTYPE";
                            ddlOilType.DataTextField = "OILTYPE";
                            ddlOilType.DataBind();
                            ddlOilType.Items.Insert(0, new ListItem("--Select Oil Type--", ""));
            ...
            private void SAVE()
            {
                ...
                try
                {
                    ...
                    insertStatement1 = "Insert into Transformer_Issue(ChallanNo, ChallanDate, Division,DivisionID, ModifiedBy)values('@ChallanNo', '@ChallanDate',  '@Division',  '#DivisionID', '@ModifiedBy')";
                    sb1.Append(insertStatement1.Replace("@ChallanNo", _tbChalanNo.Text.TrimEnd()).Replace("@ChallanDate", _tbChallanDate.Text).Replace("@Division", _ddIntendDivisions.SelectedItem.ToString()).Replace("#DivisionID", _ddIntendDivisions.SelectedValue).Replace("@ModifiedBy", Session["USERID"].ToString()));


                    //multiple insert statements for details table
                    insertStatement2 = "INSERT INTO Transformer_IssueDetails(challanNo,TransformerID, transformer, Oil, OilType) values('@challanNo','@TransformerID','@transformer', '@Oil', '#OilType')";

                    DropDownList ddlTransfomer, ddlOilType; TextBox tbOil;
                    for (int i = 0; i < gvItems.Rows.Count; i++)
                    {
                        ddlTransfomer = gvItems.Rows[i].FindControl("ddlTransfomer") as DropDownList;
                        ddlOilType = gvItems.Rows[i].FindControl("ddlOilType") as DropDownList;
                        tbOil = gvItems.Rows[i].FindControl("tbOil") as TextBox;
                        //SAVE CODE
                        if (ddlTransfomer.SelectedValue.ToString() != "")
                            sb2.Append(insertStatement2.Replace("@challanNo", _tbChalanNo.Text.TrimEnd()).Replace("@TransformerID", ddlTransfomer.SelectedValue).Replace("@transformer", ddlTransfomer.SelectedItem.ToString()).Replace("@Oil", tbOil.Text).Replace("#OilType", ddlOilType.SelectedItem.ToString()));
                    }
                    //now save it to db
                    ...
                    if (sb2.ToString() != "")
                    {
                        ...
                        lblSuccess.Text = "Issue Entry successfully added.";
