    ...
    <div style="margin:0px auto;padding:10px">
        ...
        <p class="element2 input-group custom-search-form" style="width: 80%;">
            ...
            <span style="float: left; width: 55%">
                <asp:DropDownList CssClass="form-control select-results-color"  ID="ddlItems" Width="385px" Height="35px" runat="server" AppendDataBoundItems="true" AutoPostBack="true"  DataSourceID="dataSourceItems" title="Only items received are shown here" DataTextField="itemname" DataValueField="itemid" OnSelectedIndexChanged="ddlItems_SelectedIndexChanged">
        ...
        <div style="width: 860px; overflow: auto;">
            ...
            <%-- <asp:SqlDataSource ID="dataSourceItemsInventory" runat="server" ConnectionString="<%$ ConnectionStrings:PowerDeptNagalandIMSConnectionString %>" SelectCommand="SELECT ItemID, IssueHeadName, GrossBalance, TemporaryIssued, NetActualBalance, MinimumUnitIndicator, ItemsInventoryID FROM ItemsInventory GROUP BY IssueHeadName, GrossBalance, TemporaryIssued, NetActualBalance, MinimumUnitIndicator, ItemID, ItemsInventoryID" FilterExpression="itemname = {0}" UpdateCommand="UPDATE ItemsInventory SET MinimumUnitIndicator = @MinimumUnitIndicator WHERE (ItemsInventoryID = @ItemsInventoryID)">
                    <FilterParameters>
            ...
            <asp:GridView ID="gvItemsInventory" ShowFooter="True" OnRowCommand="gvItemsInventory_RowCommand" OnRowDataBound="gvItemsInventory_RowDataBound" OnRowUpdating="gvItemsInventory_RowUpdating" Width="95%" GridLines="None" runat="server" AllowPaging="True" AllowSorting="True" AutoGenerateColumns="False">
